...
class TasksList {
    ...
    TasksList (TaskBundle tsBd){
        ...
        while (it.hasNext()) {
            ...
            //  "[^\\w\\s]",""      "[-+.^:,]",""
            String memory = taskSplit[0].replaceAll(".1.3.6.1.2.1.2.2.1.8.","");
            ...
            Task1 t = new Task1(memory, taskSplit[1],Integer.parseInt(taskSplit[2]));
            tasksList.add(t);
        }    
...
class TaskBundle {
    ...
    public TaskBundle() {
        try {
          String line;
          Process p = Runtime.getRuntime().exec("SnmpWalk.exe -r:10.4.64.8 -t:10 -c:pblc#KCG2*r -os:.1.3.6.1.2.1.2.2.1.8 -op:.1.3.6.1.2.1.2.2.1.9 -csv");
